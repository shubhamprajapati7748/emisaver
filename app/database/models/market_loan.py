...
class MarketLoan(Base):
    ...
    min_loan_amount : Mapped[int] = mapped_column(Integer, nullable=True)
    max_loan_amount : Mapped[int] = mapped_column(Integer, nullable=True)
    eligibility_criteria : Mapped[str] = mapped_column(String(500), nullable=True)
    processing_fee : Mapped[int] = mapped_column(Integer, nullable=True)
